...
namespace RootMotion.FinalIK {
	...
	public class Finger {
		...
		/// </summary>
		public bool IsValid(bool log, Transform context) {
			if (bone1 == null || bone2 == null || tip == null) {
				if (log) Warning.Log("One of the bones in the Finger Rig is null, can not initiate solvers.", context);
				return false;
			}
	...
	public class FingerRig : SolverManager {
		...
		public bool IsValid(bool log) {
			foreach (Finger finger in fingers) {
				if (!finger.IsValid(log, transform)) return false;
			}
			return true;
		}
